...
echo "$($PSQL "SELECT DISTINCT major FROM majors LEFT JOIN students ON majors.major_id = students.major_id WHERE first_name ILIKE '%ma%' AND students.major_id IS NOT NULL OR students.major_id IS NULL ORDER BY major;")"
